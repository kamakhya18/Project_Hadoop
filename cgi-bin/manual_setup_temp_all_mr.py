...
ip_tasktrackers = data.getvalue('ip_tasktrackers')


ip_details=[]

ip_tasktrackers = ip_tasktrackers.replace("\\n","\n")

commands.getoutput("sudo chmod 777 /tmp/ip_list")

f = open('/tmp/ip_list','w+')
f.write(ip_jobtracker+"\n"+ip_tasktrackers)
f.close()
...
if len(ip_list)>0:
 ...
 print "<form action='manual_setup_start_all_mr.py' method='GET'>"
 ...
 print "<input type='hidden' value="+ip_jobtracker+" name='ip_jobtracker'>"
 ...
 print "<br>"
 print "<br>"
print "<a href='manual_setup_temp_all_mr.py'>"
