...
commands.getoutput("sudo sshpass -p 'redhat' scp root@"+ip_namenode+":/etc/hadoop/hdfs-site.xml /tmp/hdfs-site1.xml")

commands.getoutput("sudo chmod 777 /tmp/hdfs-site1.xml")

f = open("/tmp/hdfs-site1.xml","r+")
temp = f.read()
temp = temp.replace("</configuration>","\n<property>\n<name>dfs.http.address</name>\n<value>"+ip_namenode+":"+port_no+"</value>\n</property>\n<property>\n<name>dfs.http.port</name>\n<value>"+port_no+"</value>\n</property>\n</configuration>")
f.close()
...
commands.getoutput("sudo sshpass -p 'redhat' ssh root@"+ip_namenode+" hadoop dfsadmin -refreshNodes")

commands.getoutput("sudo sshpass -p 'redhat' ssh root@"+ip_namenode+" hadoop-daemon.sh stop namenode")

commands.getoutput("sudo sshpass -p 'redhat' ssh root@"+ip_namenode+" hadoop-daemon.sh start namenode")

print "New Port number of namenode is "+port_no
